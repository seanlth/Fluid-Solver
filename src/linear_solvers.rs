...
pub fn relaxation(a: fn(i32, i32, i32) -> f64, x: &mut Vec<f64>, b: &Vec<f64>, w: i32, h: i32, density: f64, dt: f64, dx: f64, limit: usize) {
    let n = w*h;
    ...
    for _ in 0..limit {
        for i in 0..n {
            ...
            temp[i as usize] = ( b[i as usize] + p1 + p2 + p3 + p4 ) / (4.0 * (dt / ( density * dx * dx )));
        }
        *x = temp.clone();
    }
}
